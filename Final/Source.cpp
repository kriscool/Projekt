...
void zapis(std::string path) {

	Mat sudoku = imread(path, 0);
	Mat zrodlo = imread(path, 1);
	Mat src_gray;
	...
	cv::imwrite("Wycinka\\pokaz.jpg", sudoku);
	...
	for (int j = 0; j<9; j++) {
		...
		for (int i = 0; i<9; i++) {
			...
			{
				photocount++;
				imagename = "Wycinka\\" + inttostr(photocount) + ".jpg"; //sciezka do zapisu pliku
