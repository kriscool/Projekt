...
void SudokuRead::cornerHarris_demo(int, void*, Mat src, Mat src_gray)
{
...
void SudokuRead::drawLine(Vec2f line, Mat &img, Scalar rgb = CV_RGB(0, 0, 255))
{
	if (line[1] != 0)
	{
...
void SudokuRead::mergeRelatedLines(vector<Vec2f> *lines, Mat &img)
{
...
String SudokuRead::inttostr(int input)
{
	stringstream ss;
	ss << input;
	return ss.str();
...
int SudokuRead::go()
{
	...
	s.mergeRelatedLines(&lines, sudoku);
	...
	s.drawLine(topEdge, sudoku, CV_RGB(0, 0, 0));
	s.drawLine(bottomEdge, sudoku, CV_RGB(0, 0, 0));
	s.drawLine(leftEdge, sudoku, CV_RGB(0, 0, 0));
	s.drawLine(rightEdge, sudoku, CV_RGB(0, 0, 0));
	...
	s.cornerHarris_demo(0, 0, zrodlo, src_gray);
